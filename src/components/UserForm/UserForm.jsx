...
const UserForm = () => {
  ...
  const validationSchema = Yup.object().shape({
    name: Yup.string().trim().required('Reqired'),
    ...
    birthday: Yup.date().max(eighteenYearsAgo, 'must be older then 18 years').required('Reqired'),
  ...
  return (
    <FormStyled
      onSubmit={e => {
        e.preventDefault();
        dispatch(profileOperations.profileDataUpdate(values));
      }}
    >
